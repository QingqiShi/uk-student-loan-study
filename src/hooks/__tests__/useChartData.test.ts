...
describe('useChartData hooks', () => {
  const expectedDataPoints = Math.floor((MAX_SALARY - MIN_SALARY) / SALARY_STEP) + 1;
  ...
  describe('useTotalRepaymentData', () => {
    ...
    it('starts at MIN_SALARY and ends at MAX_SALARY', () => {
      ...
      expect(result.current.data[0][0]).toBe(MIN_SALARY);
      expect(result.current.data[result.current.data.length - 1][0]).toBe(MAX_SALARY);
    });
    ...
    it('calculates higher repayment for higher earners who pay off loan', () => {
      ...
      const highSalaryRepayment = result.current.data[result.current.data.length - 1][1];
  ...
  describe('useRepaymentYearsData', () => {
    ...
    it('uses offset of 5000 for annotation boundary', () => {
      ...
      const { result: withAnnotation } = renderHook(() => useRepaymentYearsData());
      ...
      const { result: withoutAnnotation } = renderHook(() => useRepaymentYearsData());
    ...
    it('higher salary generally means fewer years to pay off', () => {
      ...
      const highSalaryYears = result.current.data[result.current.data.length - 1][1];
